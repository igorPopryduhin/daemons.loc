...
namespace Engine\Daemon;

use Engine\DaemonException\DaemonException;
use Engine\DI\DI;
use Engine\Model;
...
abstract class DaemonPHP
{
     ...
     * @access public
     */
    public function __construct (DI $di, $path = null)
    {
    ...
    final protected function demonize ()
    {
        ...
        if ($pid == -1) {
            ...
            $this->fpid = fopen($this->_pid, 'wb');
            fwrite($this->fpid, $pid);
            fclose($this->fpid);
            exit(0);
        }
        ...
        if (!is_writable($this->_log)) throw new DaemonException('LOG-file is not writable!');
        $STDERR = fopen($this->_err, 'ab');
        if (!is_writable($this->_err)) throw new DaemonException('ERR-file is not writable!');
        $this->run();
    }
    ...
    final public function handle ($argv)
    {
        switch ($argv[1]) {
            ...
            case 'status':
                ...
                break;
            default:
                echo "Unknown command!" . PHP_EOL . "Use: " . $argv[0] . " start|stop|restart|status" . PHP_EOL;
                break;
        }
    }
